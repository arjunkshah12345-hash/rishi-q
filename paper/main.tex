...
for Quantum-Structural Analogues in Classical Sanskrit Thought\\[0.35em]
\normalsize (RISHI-Q)}
\author{
Arjun Shah\\
\small RISHI-Q Project\\
\small\texttt{https://github.com/arjunkshah/rishi-q}\\[0.5em]
\small Sister experimental project: RISHI-FIELDS (\texttt{https://github.com/arjunkshah/rishi-fields})
}
\date{\today}

\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small RISHI-Q preprint}
...
This manuscript documents the scientific design, ontology, pipelines, reproducibility machinery, instrument validation on synthetic and modern-physics controls, an exploratory public-domain pilot ($n=276$), a Capra-style claim autopsy (5/5 claims \texttt{CONTRADICTED\_AS\_QUANTUM} on the Ved\={a}nta PD slice), and System~B discovery outputs under a heuristic annotator.
It does \textbf{not} claim confirmatory Sanskrit results, historical discovery of quantum mechanics, or Tier-5 transformative discoveries.
A negative confirmatory result remains a successful scientific outcome.
Physical detection of pr\={a}\d{n}a or related concepts is out of scope and is pursued separately in RISHI-FIELDS.
\end{abstract}

\noindent\textbf{Keywords:} computational humanities; history of physics; Sanskrit philosophy; quantum foundations; blinded annotation; motif discovery; falsifiability
...
\input{tables/tab_hard_rules.tex}
...
\label{fig:firewall}
\end{figure}
\input{tables/tab_project_status.tex}
...
\end{figure}
\input{tables/tab_positive_controls.tex}
...
Open-model annotation on Kaggle was subsequently authenticated under account \texttt{aks1321}; early GPU batch workers did not attach accelerators.
A CPU oneshot pilot (Qwen2.5-0.5B) returned uniformly \texttt{NA} labels (model too weak for 36-feature oneshot JSON).
A batched follow-up ($6$ features/call; $80$ passages; prompt \texttt{ann-v0.3-batched6}) completed with $2875$ \texttt{NA}, $5$ explicit \texttt{0}, and $0$ positive labels---still insufficient for Level~III inference, and treated as an annotator-capacity limit rather than a Sanskrit null.
GPU unlock (phone verification / UI accelerator) and larger instruct models remain on the exploratory path.
The confirmatory firewall is unchanged.
...
The present manuscript freezes the methodological story, demonstrates instrument behavior on controls, and reports exploratory System~B outputs and Capra-style claim autopsies without manufacturing significance or inventing ancient quantum discovery.
Under heuristic public-domain annotation, Level~I metaphysical structure appears without Level~III quantum enrichment for the Ved\={a}nta slice examined here---a result that constrains popular rhetoric even while confirmatory H1 remains locked.
Confirmatory evaluation awaits preregistration, human validation, and stronger annotation backends.
\textbf{Rigor first; discovery second; extraordinary claims only with extraordinary evidence.}

\section*{Data and code availability}
Software: MIT-licensed repository (\texttt{https://github.com/arjunkshah/rishi-q}).
Private blinding maps are not published.
Regenerate exhibits:
\begin{verbatim}
uv run python scripts/run_discovery_engine.py
uv run python scripts/run_capra_autopsy.py
...
No human subjects were enrolled for this textual study.
...
\input{tables/tab_ontology_features.tex}
\section{Exploratory passage scores}
\input{tables/tab_passage_scores.tex}
\section{Frequent positive features}
\input{tables/tab_positive_features.tex}
